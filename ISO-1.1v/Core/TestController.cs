...
public class TestController
{
    ...
    private int _lastTerminationCheckMinute = -1; // 防止同一分钟多次检查
    ...
    private void CheckTerminationCondition()
    {
        ...
        if (CurrentTest.DurationMode == "Standard" && secs >= 1800 && secs % 300 < 1)
        {
            int checkMinute = secs / 300;
            if (checkMinute == _lastTerminationCheckMinute) return;
            _lastTerminationCheckMinute = checkMinute;
            double drift = DriftCalculator.CalculateDrift(_driftHistory);
            double maxDrift = _simConfig.MaxTemperatureDriftPerTenMinutes;
            if (!double.IsNaN(drift) && Math.Abs(drift) <= maxDrift)
            {
